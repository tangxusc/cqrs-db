...
func BindParameter(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringVarP(&Instance.Proxy.Address, "proxy-address", "", "127.0.0.1", "proxy数据库连接地址")
	cmd.PersistentFlags().StringVarP(&Instance.Proxy.Port, "proxy-port", "", "3306", "proxy数据库端口")
	cmd.PersistentFlags().StringVarP(&Instance.Proxy.Database, "proxy-Database", "", "", "proxy数据库实例")
	cmd.PersistentFlags().StringVarP(&Instance.Proxy.Username, "proxy-Username", "", "", "proxy数据库用户名")
	cmd.PersistentFlags().StringVarP(&Instance.Proxy.Password, "proxy-Password", "", "", "proxy数据库密码")
